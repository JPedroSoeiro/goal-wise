...
// Não precisamos de props aqui, pois é um chat geral
const ChatGeralRoom = () => {
  ...
  useEffect(() => {
    ...
    // Conecta ao WebSocket sem um teamId
    const websocket = new WebSocket(`ws://192.168.1.130:3000`);
    setWs(websocket);

    websocket.onopen = () => {
      console.log("WebSocket geral conectado!");
    ...
    websocket.onclose = () => {
      console.log("WebSocket geral desconectado.");
    };
  ...
  const sendMessage = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (content && session?.user && ws) {
      const message: ChatMessage = {
        ...
        userColor: "#8884d8", // Pode definir cores diferentes se quiser
        content: content,
      };
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] w-full flex-col">
      <div className="flex items-center justify-between border-b p-3">
        <h2 className="text-lg font-bold">Chat Geral</h2>
